...
const Rank_list = () => {
  const [rankedActivePlayers, setRankedActivePlayers] = useState<[{first_name: string, last_name: string, fantasy_data_id: number, fantasy_positions: string, injury_status: string}]>()
  ...
  const playerlist = rankedActivePlayers?.map((player, index) => { 
    ...
    return <PlayerCard index={num} first={first_name} last={last_name} key={id} id={id} pos={pos} inj={inj}/> 
  })

  return (
    <Box sx={{width:'50%', m:'auto',p: 0, bgcolor: '#EAEAEA'}}>
      <List sx={{width:'90%', m:'auto',p: 0 }}>
          {playerlist}
      </List>
    </Box>
  )
